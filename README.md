[![PyPI version](https://badge.fury.io/py/kooker.svg)](https://badge.fury.io/py/kooker)
[![Build Status](https://jenkins.eosc-synergy.eu/buildStatus/icon?job=indigo-dc%2Fkooker%2Fmaster)](https://jenkins.eosc-synergy.eu/job/indigo-dc/job/kooker/job/master/)
...
kooker is a basic user tool to execute simple docker containers in user
...
kooker is a wrapper around several tools and libraries to mimic a
subset of the docker capabilities including pulling images and running
containers with minimal functionality.

**Important notice: We have changed the kooker tools location as of kooker 1.3.17.
This affects the configuration option `conf['tarball']` and environment variable
`KOOKER_TARBALL`, so if you are using kooker <= 1.3.16, make sure to:
`export KOOKER_TARBALL=https://download.a.incd.pt/kooker/kooker-englib-1.2.11.tar.gz`.**

## Documentation

The full documentation is available at:

* [kooker documentation](https://indigo-dc.github.io/kooker/)
* [Installation manual](https://indigo-dc.github.io/kooker/installation_manual.html)
* [User manual](https://indigo-dc.github.io/kooker/user_manual.html)
* [Reference card](https://indigo-dc.github.io/kooker/reference_card.html)

## How does it work

kooker is written in Python, it has a minimal set of dependencies so
that can be executed in a wide range of Linux systems.

kooker does not make use of docker nor requires its presence.
...
kooker transparently supports several methods to execute the containers
...
execution are downloaded and deployed by kooker during the installation
process. This installation is performed in the user home directory
and does not require privileges. The kooker related files such as
libraries, executables, documentation, licenses, container images and
extracted directory trees are placed by default under `$HOME/.kooker`.
...
Since v1.3.0, kooker supports Python 2.7 and Python >= 3.6.
The original kooker v1.1.x for Python 2 is no longer maintained
but is still available
[here](https://github.com/indigo-dc/kooker/tree/v1.1.8).
...
```bash
kooker search  fedora
kooker search  ubuntu
kooker search  debian

kooker search --list-tags ubuntu
```

Pull from dockerhub and list the pulled images.

```bash
kooker pull   fedora:39
kooker pull   busybox
...
```bash
kooker search  quay.io/bio
kooker search  --list-tags  quay.io/biocontainers/scikit-bio
kooker pull    quay.io/biocontainers/scikit-bio:0.2.3--np112py35_0
kooker images
```
...
```bash
kooker run -v /home/u457:/home/cuser -w /home/user myfed  /bin/bash
kooker run -v /var -v /proc -v /sys -v /tmp  myfed  /bin/bash
```
...
```bash
kooker run  -v /tmp  --entrypoint="" myfed  /bin/bash -c 'cd /tmp; ./myscript.sh'

kooker run  -v /tmp  --entrypoint=/bin/bash  myfed  -c 'cd /tmp; ./myscript.sh'
```
...
kooker run --user root   myfed  /bin/id
kooker run --user jorge  myfed  /bin/id
```
...
```bash
kooker setup  --execmode=F3  myfed
...
```bash
kooker setup  --execmode=P1  myfed
```

Change execution engine to runc.

```bash
kooker setup  --execmode=R1  myfed
```
...
```bash
./kooker setup  --execmode=S1  myfed
```
...
```bash
kooker setup  --nvidia  mytensorflow
```

## Security

By default kooker via PRoot offers the emulation of the root user. This
...
home directory under `$HOME/.kooker` or other location of choice. Therefore
...
invoking it. Users can downloaded kooker and execute it without requiring
system administrators intervention.

kooker also provides execution with runc, crun and Singularity, these modes
...
When executed by normal unprivileged users, kooker limits privilege
...
kooker is not meant to create containers. Creation of containers
is better performed using docker and dockerfiles.

kooker does not provide all the docker features, and is not intended
as a docker replacement.

kooker is mainly oriented at providing a run-time environment for
containers execution in user space. kooker is particularly suited to
...
kooker offers multiple execution modes leveraging several external tools
...
```bash
./kooker setup  --execmode=P2  my-container-id
```

The Fakechroot modes (Fn modes) require shared libraries compiled against
the libc shipped with the container. kooker provides these libraries for
several Linux distributions, these shared libraries are installed by
kooker under:

```bash
$HOME/.kooker/lib/libfakechroot-*
```
...
the host system. Singularity is not shipped with kooker.
...
```bash
codemetapy kooker --with-orcid --affiliation "LIP Lisbon" \
  --buildInstructions "https://https://github.com/indigo-dc/kooker/blob/master/docs/installation_manual.md#3-source-code-and-build" \
  --citation "https://doi.org/10.1016/j.cpc.2018.05.021" \
  --codeRepository "https://github.com/indigo-dc/kooker" \
  --contIntegration "https://jenkins.eosc-synergy.eu/job/indigo-dc/job/kooker/job/master/" --contributor "Mario David" \
  --copyrightHolder "LIP"  --copyrightYear "2016" --creator "Jorge Gomes" \
  --dateCreated "2021-05-26" --maintainer "Jorge Gomes" \
  --readme "https://github.com/indigo-dc/kooker/blob/master/README.md" \
  --referencePublication "https://doi.org/10.1016/j.cpc.2018.05.021" \
  --releaseNotes "https://github.com/indigo-dc/kooker/blob/master/changelog" \
  -O codemeta.json
```
...
When citing kooker please use the following:
...
* Jorge Gomes, Emanuele Bagnaschi, Isabel Campos, Mario David,
  ...
  Enabling rootless Linux Containers in multi-user environments: The kooker
...
shipped with kooker. These include the [kooker license](LICENSE) and the
individual licences of the external tools and libraries packaged for use
with kooker. For further information see the
[software licenses section](https://indigo-dc.github.io/kooker/installation_manual.html#62-software-licenses)
