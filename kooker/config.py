...
class Config(object):
    ...
    Config.init() and that can reside in ~/.kooker/kooker.conf
    """
    conf = {}
    conf['verbose_level'] = 3
    conf['homedir'] = os.path.expanduser("~") + "/.kooker"
    ...
    # kooker installation tarball the release is the minimum requirement
    ...
    # KOOKER = use executable from the kooker binary distribution/tarball
    conf['use_proot_executable'] = "KOOKER"
    conf['use_patchelf_executable'] = "KOOKER"
    ...
    def _env_override(self):
        ...
        Config.conf['verbose_level'] = \
            int(os.getenv("KOOKER_LOGLEVEL", Config.conf['verbose_level']))
        Config.conf['topdir'] = os.getenv("KOOKER_DIR", Config.conf['topdir'])
        Config.conf['bindir'] = os.getenv("KOOKER_BIN", Config.conf['bindir'])
        Config.conf['libdir'] = os.getenv("KOOKER_LIB", Config.conf['libdir'])
        Config.conf['docdir'] = os.getenv("KOOKER_DOC", Config.conf['docdir'])
        Config.conf['reposdir'] = \
            os.getenv("KOOKER_REPOS", Config.conf['reposdir'])
        Config.conf['layersdir'] = \
            os.getenv("KOOKER_LAYERS", Config.conf['layersdir'])
        Config.conf['containersdir'] = \
            os.getenv("KOOKER_CONTAINERS", Config.conf['containersdir'])
        Config.conf['dockerio_index_url'] = \
            os.getenv("KOOKER_INDEX", Config.conf['dockerio_index_url'])
        Config.conf['dockerio_registry_url'] = \
            os.getenv("KOOKER_REGISTRY", Config.conf['dockerio_registry_url'])
        Config.conf['tarball'] = \
            os.getenv("KOOKER_TARBALL", Config.conf['tarball'])
        Config.conf['override_default_execution_mode'] = \
            os.getenv("KOOKER_DEFAULT_EXECUTION_MODE",
                      Config.conf['override_default_execution_mode'])
        Config.conf['fakechroot_so'] = \
            os.getenv("KOOKER_FAKECHROOT_SO", Config.conf['fakechroot_so'])
        Config.conf['fakechroot_libc'] = \
            os.getenv("KOOKER_FAKECHROOT_LIBC", Config.conf['fakechroot_libc'])
        Config.conf['tmpdir'] = os.getenv("KOOKER_TMP", Config.conf['tmpdir'])
        Config.conf['keystore'] = \
            os.getenv("KOOKER_KEYSTORE", Config.conf['keystore'])
        Config.conf['use_curl_executable'] = \
            os.getenv("KOOKER_USE_CURL_EXECUTABLE",
                      Config.conf['use_curl_executable'])
        Config.conf['use_proot_executable'] = \
            os.getenv("KOOKER_USE_PROOT_EXECUTABLE",
                      Config.conf['use_proot_executable'])
        Config.conf['use_runc_executable'] = \
            os.getenv("KOOKER_USE_RUNC_EXECUTABLE",
                      Config.conf['use_runc_executable'])
        Config.conf['use_singularity_executable'] = \
            os.getenv("KOOKER_USE_SINGULARITY_EXECUTABLE",
                      Config.conf['use_singularity_executable'])
        Config.conf['use_patchelf_executable'] = \
            os.getenv("KOOKER_USE_PATCHELF_EXECUTABLE",
                      Config.conf['use_patchelf_executable'])
        Config.conf['fakechroot_cmd_subst'] = \
            os.getenv("KOOKER_FAKECHROOT_CMD_SUBST",
                      Config.conf['fakechroot_cmd_subst'])

        Config.conf['fakechroot_expand_symlinks'] = \
            os.getenv("KOOKER_FAKECHROOT_EXPAND_SYMLINKS",
