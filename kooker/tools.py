...
class KookerTools(object):
    """Download and setup of the kooker supporting tools
    ...
    def _instructions(self):
        """
        Kooker installation instructions are available at:

          https://github.com/indigo-dc/kooker/tree/master/doc
          https://github.com/indigo-dc/kooker/tree/devel/doc

        Kooker requires additional tools to run. These are available
        in the kooker tarball. The tarballs are available at several
        locations. By default kooker will install from the locations
        defined in Config.tarball.

        To install from files or other URLs use these instructions:

        1) set KOOKER_TARBALL to a remote URL or local filename:

          $ export KOOKER_TARBALL=http://host/path
          or
          $ export KOOKER_TARBALL=/tmp/filename

        2) run kooker with the install command or optionally using
        ...
        3) once installed the binaries and containers will be placed
           by default under $HOME/.kooker

        """
        Msg().out(self._instructions.__doc__, l=Msg.ERR)
        Msg().out("kooker command line interface version:", __version__,
                  "\nrequires kookertools tarball release :",
    ...
    def _download(self, url):
        """Download a file """
        download_file = FileUtil("kookertools").mktmp()
    ...
    def _verify_version(self, tarball_file):
        ...
        try:
            ...
            for tar_in in tfile.getmembers():
                if tar_in.name.startswith("kooker_dir/lib/VERSION"):
    ...
    def _clean_install(self, tfile):
        ...
        for tar_in in tfile.getmembers():
            ...
            if tar_in.name.startswith("kooker_dir/bin/"):
                f_path = self.localrepo.bindir + '/' + basename
                FileUtil(f_path).register_prefix()
                FileUtil(f_path).remove(recursive=True)
            if tar_in.name.startswith("kooker_dir/lib/"):
                f_path = self.localrepo.libdir + '/' + basename
                FileUtil(f_path).register_prefix()
                FileUtil(f_path).remove(recursive=True)
            if tar_in.name.startswith("kooker_dir/doc/"):
    ...
    def _install_logic(self, force=False):
        ...
        for url in self._get_mirrors(self._tarball):
            ...
            if status:
                Msg().out("Info: installing kookertools", version)
                status = self._install(tarballfile)
            elif force:
                Msg().out("Info: forcing install of kookertools", version)
                status = self._install(tarballfile)
            else:
    ...
    def install(self, force=False):
        """Get the kooker tools tarball and install the binaries"""
        ...
        if not self._tarball:
            Msg().out("Info: KOOKER_TARBALL not set, installation skipped",
                      l=Msg.VER)
            return True

        Msg().out("Info: kooker command line interface", __version__)
        Msg().out("Info: searching for kookertools >=", self._tarball_release, l=Msg.INF)
        retry = self._installretry
        while retry:
            if self._install_logic(force):
                self.get_installinfo()
                Msg().out("Info: installation of kookertools successful")
                return True
        ...
        self._instructions()
        self.get_installinfo()
        Msg().err("Error: installation of kookertools failed")
        return False
