...
class Uprocess(object):
    ...
    def get_output(self, cmd, ignore_error=False):
        ...
        content = ""
        try:
            content = self.check_output(cmd, shell=False, stderr=Msg.chlderr, close_fds=True)
    ...
    def pipe(self, cmd1, cmd2, **kwargs):
        ...
        try:
            proc_2 = subprocess.Popen(cmd2, stderr=Msg.chlderr, shell=False, stdin=proc_1.stdout)
        except (OSError, ValueError):
            proc_1.kill()
            return False
