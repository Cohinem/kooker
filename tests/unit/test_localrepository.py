#!/usr/bin/env python
"""
kooker unit tests: LocalRepository
"""

from unittest import TestCase, main
from unittest.mock import patch, mock_open, call
from kooker.container.localrepo import LocalRepository
...
class LocalRepositoryTestCase(TestCase):
    ...
    def setUp(self):
        Config().getconf()
        Config().conf['topdir'] = KOOKER_TOPDIR
        Config().conf['bindir'] = ""
        Config().conf['libdir'] = ""
    ...
    def test_01_init(self, mock_fu):
        ...
        self.assertEqual(lrepo.topdir, KOOKER_TOPDIR)
    ...
    def test_02_setup(self, mock_fu):
        ...
        newdir = "/home/u2/.kooker"
        lrepo = LocalRepository(KOOKER_TOPDIR)
        lrepo.setup(newdir)
        self.assertEqual(lrepo.topdir, newdir)

    @patch('kooker.container.localrepo.os.path.exists')
    @patch('kooker.container.localrepo.os.makedirs')
    @patch('kooker.container.localrepo.FileUtil')
    ...
    @patch('kooker.container.localrepo.FileUtil.iswriteable')
    @patch.object(LocalRepository, 'cd_container')
    @patch('kooker.container.localrepo.FileUtil.isdir')
    @patch('kooker.container.localrepo.os.path.exists')
    def test_12_iswriteable_container(self, mock_exists,
        ...
        mock_cdcont.return_value = "/home/u1/.kooker/containerid"
        lrepo = LocalRepository(KOOKER_TOPDIR)
    ...
    @patch.object(LocalRepository, 'cd_container')
    @patch('kooker.container.localrepo.Uprocess.get_output')
    @patch('kooker.container.localrepo.FileUtil')
    ...
    @patch('kooker.container.localrepo.os.listdir')
    @patch('kooker.container.localrepo.os.path.isdir')
    @patch('kooker.container.localrepo.FileUtil')
    ...
    def test_15_del_container(self, mock_fu,
        ...
        mock_fu.return_value.register_prefix.side_effect = \
            [None, None, None]
        cont_id = "d2578feb-acfc-37e0-8561-47335f85e46a"
        cdirs = "/home/u1/.kooker/containers"
    ...
    @patch('kooker.container.localrepo.os.path.relpath')
    @patch('kooker.container.localrepo.os.symlink')
    @patch('kooker.container.localrepo.os.path.exists')
    @patch('kooker.container.localrepo.FileUtil')
    ...
    @patch('kooker.container.localrepo.os.path.islink')
    @patch('kooker.container.localrepo.FileUtil')
    ...
    @patch('kooker.container.localrepo.os.readlink')
    @patch('kooker.container.localrepo.os.path.isdir')
    @patch('kooker.container.localrepo.os.path.islink')
    @patch('kooker.container.localrepo.FileUtil')
    ...
    @patch('kooker.container.localrepo.os.path.isfile')
    @patch('kooker.container.localrepo.FileUtil')
    ...
    @patch.object(LocalRepository, '_symlink')
    @patch('kooker.container.localrepo.os.path.basename')
    @patch('kooker.container.localrepo.os.path.islink')
    @patch('kooker.container.localrepo.os.path.exists')
    @patch('kooker.container.localrepo.FileUtil')
    ...
    @patch('kooker.container.localrepo.json.loads')
    @patch('kooker.container.localrepo.os.path.exists')
    @patch('kooker.container.localrepo.FileUtil')
